...
A machine-readable reference of field-tested Claude Code operational patterns. The primary use case: Claude Code on a different machine ingests this document and produces a gap analysis of that machine's setup — identifying missing configuration, underused patterns, and known traps.

This is not a tutorial, getting-started guide, or configuration reference (the official docs cover those). It captures the decisions, traps, and patterns that aren't in the docs and that cost real time to discover. Every section answers: "what would an engineer who already uses Claude Code daily learn here that would change how they configure or operate it?"
...
- **Event-triggered** — Hooks. They run scripts in response to tool use or session events. SessionStart and UserPromptSubmit stdout becomes conversation context; other hook types do not by default.
...
**`model: opus` in agent frontmatter** is an alias that resolves to whichever Opus version that CLI build maps it to — usually but not necessarily the latest. For stability, pin a specific model ID; for staying current, use the alias.
...
1. **Idle time** — the cache TTL is 5 minutes by default (1 hour on some plans). Walking away for lunch re-pays the full prefix. This is the most common cache killer.
...
The schema is not fully documented; dump stdin once on first setup and inspect. The script runs frequently, so it must be fast — do all JSON parsing in a single `jq` call.
...
2. **Rate limit proximity** — shows 5-hour and 7-day usage percentages with reset countdown when usage is high.
...
**Pattern: evidence-first commands.** If the model can see git commits, file changes, and completed items, the command should synthesize from evidence and present for confirmation — not ask the user to narrate what happened.

**Pattern: read-only scan → report → confirmed fixes.** For any structural sweep (index gaps, orphan files, config drift), the command reads first and reports findings, then asks before acting. Never auto-deletes.
...
This is the most transferable concept from the agent system.
...
The key insight is that the template is a real file on disk (not instructions in CLAUDE.md), so it stays out of the context window until needed and doesn't drift the way inline instructions do.
...
**Feedback memories** — behavioral corrections that prevent repeating mistakes. Examples: "use worked examples with visible data in knowledge notes, not just definitions"; "synthesize from git evidence instead of asking the user to narrate what happened."

**User profile** — role, goals, tech stack. Changes how explanations are framed and what level of detail is appropriate.
...
MCP (Model Context Protocol) servers extend Claude Code with external tool access. The non-obvious parts:
...
6. **Forks for verification.** Forks inherit the builder's context and biases. Verification agents must be fresh (zero context, self-contained prompt) to provide independent judgment. Any quality gate should be a fresh agent. (§9)
...
11. **Treating memory as documentation.** Memory is a stale cache of behavioral corrections. Architecture, file paths, code patterns — these are derivable from the codebase and become stale in memory. If it can be `grep`'d, it shouldn't be memorized. (§11)
...
- [ ] Global `CLAUDE.md`: output style, behavioral preferences — concise, every line earns its context cost
